...
# ================= UTILITIES =================
def create_run_dir(base_dir=REPORTS_BASE):
...
# ================= MODEL =================
def load_model(model_path=MODEL_PATH, cache_dir=HF_CACHE_DIR):
...
def run_single_evaluation(
    ...
    tweet_loader=lambda: load_from_disk(TWEET_PATH),
    yt_loader=lambda: load_from_disk(YT_PATH),
    report_base=REPORTS_BASE
):
    print("[Monitoring] Starting evaluation")

    run_dir = create_run_dir(report_base)

    model = model_loader()
    tweet_ds = tweet_loader()
    yt_ds = yt_loader()
    ...
    history_file = os.path.join(report_base, "history.json")
    history = []
